...
def rate_movie_view(request):
    ...
    if user.is_authenticated:
        message = "<span class='bg-danger text-light py-1 px-3 rounded'>An error occurred.</span>"
        ...
        if rating_obj.content_object is not None:
            message = "<span class='bg-success text-light py-1 px-3 rounded'>Rating Saved!</span>"
            response = HttpResponse("Skipping", status=200)
